...
## MANDATORY: /walkthrough gate

Before ANY red team validation, check if `workspaces/<project>/04-validate/user-flow-walkthrough.md` exists and has zero BLOCKING gaps. If it does not exist or has BLOCKING gaps:

1. **STOP** — do not proceed with red team
2. **Run `/walkthrough` first** — click through every screen on both platforms
3. **Fix all BLOCKING gaps** found during walkthrough
4. Only then proceed to red team

**Why this gate exists:** The pattern of testing features in isolation (API returns 200, component renders) and declaring "PASS" — without ever clicking through the app as a user — was repeatedly caught by the human. The walkthrough forces you to use the product before certifying it.
